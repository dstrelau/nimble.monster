 ...
 * RuleMarkdown renders as interactive DiceNotation components.
 */
...
/** Resolve every placeholder form in a rule body. */
export async function resolveRuleContent(markdown: string): Promise<string> {
  return resolveTermLinks(
    await resolveSpellSchoolLinks(resolveDiceNotationLinks(markdown))
  );
}
