...
import { getRuleLocation, validateRuleHierarchy } from "./categories";

export interface Rule {
  /** Flat, content-matching slug. Also the URL: /rules/<slug>. */
  slug: string;
  title: string;
  /** Derived from the rule's central hierarchy location. */
  category: string;
  content: string;
  keywords: string[];
  sources?: RuleSource[];
...
export interface RuleSource {
  book: "Core Rules v2" | "GMG v1";
  pages?: number[];
}

const RULES_DIR = join(process.cwd(), "data/rules");

export function parseKeywords(slug: string, frontmatter: string): string[] {
  const match = frontmatter.match(/^keywords:\s*(\[[^\n]*\])\s*$/m);
  if (!match) throw new Error(`${slug}.mdx: missing keywords JSON array`);
  let value: unknown;
  try {
    value = JSON.parse(match[1]);
  } catch {
    throw new Error(`${slug}.mdx: keywords must be a valid JSON array`);
  }
  if (
    !Array.isArray(value) ||
    value.length === 0 ||
    ...
    throw new Error(
      `${slug}.mdx: keywords must be a nonempty array of strings`
    );
  }
  const normalized = value.map((item) => item.trim());
  if (
    new Set(normalized.map((item) => item.toLocaleLowerCase())).size !==
    normalized.length
  ) {
    throw new Error(`${slug}.mdx: keywords must be unique`);
  }
  return normalized;
}
...
export function parseSources(
  ...
  while (index < lines.length && /^\s/.test(lines[index])) {
    ...
    if (book !== "Core Rules v2" && book !== "GMG v1") {
      throw new Error(
...
function load(): { all: Rule[]; bySlug: Map<string, Rule> } {
  const parsed = readdirSync(RULES_DIR)
  ...
  validateRuleVariants(parsed);
  validateRuleHierarchy(parsed);
  const all = parsed.map((rule) => {
    const location = getRuleLocation(rule.variantOf ?? rule.slug);
    if (!location) {
      throw new Error(`${rule.slug}.mdx: missing rule hierarchy location`);
    }
    return { ...rule, category: location.category.slug };
  });
