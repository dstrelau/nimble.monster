...
export const GET = telemetry(async (request: Request) => {
  ...
  const { cursor, limit, sort, search } = result.data;
  ...
  const { data: families, nextCursor } =
    await familiesService.paginatePublicFamilies({
      cursor,
      limit,
      sort,
      search,
  ...
  const data = families.map(toJsonApiFamily);
