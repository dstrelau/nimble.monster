...
export const GET = telemetry(async (
  _request: Request,
...
export const PUT = telemetry(async (
  request: Request,
  { params }: { params: Promise<{ id: string }> },
) => {
...
export const DELETE = telemetry(async (
  _request: Request,
  { params }: { params: Promise<{ id: string }> },
) => {
  const session = await auth();
  ...
  if (existingMonster.creator.discordId !== session.user.id) {
    return NextResponse.json({ error: "Forbidden - you don't own this monster" }, { status: 403 });
  }
