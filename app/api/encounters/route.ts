...
import { toJsonApiEncounter } from "@/lib/services/encounters/converters";
...
export const GET = telemetry(async (request: Request) => {
  ...
  const { encounters, nextCursor } = await repository.listPublicEncounters({
    cursor,
    limit,
    sort,
  });

  const data = encounters.map(toJsonApiEncounter);
  ...
  if (includeCreator) {
    const included = collectCreators(encounters);
