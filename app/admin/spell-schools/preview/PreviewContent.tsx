  ...
  return (
    <div className="py-8 space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold mb-2">
            Preview Official Spell Schools
          </h1>
          <p className="text-muted-foreground">
            {items.length} spell school{items.length !== 1 ? "s" : ""} ready to
            import
            {sourceName && (
              <>
                {" "}
        ...
        <div className="flex gap-4">
          <form
            action={cancelOfficialSpellSchoolsUploadAction.bind(
              null,
              sessionKey
            )}
          >
          ...
          <form
            action={commitOfficialSpellSchoolsAction.bind(null, sessionKey)}
          >
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {filtered.map(({ spellSchool, status }) => (
          <div key={spellSchool.name} className="relative">
            <Badge
              variant="secondary"
              className={cn(
